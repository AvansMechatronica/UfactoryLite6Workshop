...
class ObjectDetector:
    def __init__(self, obj_type, obj_color, workspace_ratio=1.0, ret_image_bool=False):
        ...
        self._nb_sides = 0#obj_type.value
    ...
    def extract_object_with_hsv(self, img):
        ...
        #cv2.imshow("Work", im_work)
        if im_work is None:  # Case where working area is not found
            status = False#CommandStatus.MARKERS_NOT_FOUND
            print("Vision Node - Markers Not Found !")
            return status, result_pose, "", "", img
        ...
        if cx is None:
            print("Vision Node - No Object Found !")
        ...
        if self.should_ret_image() and status == True:#CommandStatus.SUCCESS:
            # im_draw = cv2.resize(im_draw, dsize=(450, 450))
            list_pos = [result_pose.x, result_pose.y, result_pose.z]
            im_draw = get_annotated_image_rel_pos(im_draw, list_pos, angle)

        return status, result_pose, obj_type, obj_color, im_draw
    ...
    def extract_biggest_blob(self, im_thresh):
        ...
        return cx, cy, angle, obj_type, obj_color, im_ret
